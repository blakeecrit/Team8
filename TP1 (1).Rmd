...
##### Equation 10.8
$\frac{\sum_{i=1}^{n}(\sum_{j=1}^{p} \phi_{jm} x_{ij})^{2} } { \sum_{j=1}^{p} \sum_{i=1}^{n} x^{2}_{ij}}$
...
Recall:   Percent of Variance Explained = Variance explained / Total Variance
  ...
  Note: The prcomp() function gives the standard deviation
...
##### Method 2 Solution:
  1. Find loadings using prcomp() function
  
  Note: Loadings are found using "$roatation" from the prcomp() function 
